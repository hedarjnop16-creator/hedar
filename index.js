...
// ⚠️ معرف الأدمن لتلقي الإشعارات وقبول/رفض الطلبات
...
const userBalances = {}; 
const botUserIDs = {}; 
const approvedUsers = new Set(); // قائمة الحسابات المقبولة
...
// القائمة الرئيسية (تختلف حسب حالة قبول الحساب)
function mainKeyboard(chatId) {
  const customID = getOrCreateBotID(chatId);
  const isApproved = approvedUsers.has(chatId);

  let keyboard = [];

  if (isApproved) {
    // الزبون المقبول يظهر له الشحن والسحب
    keyboard = [
    ...
    // الزبون الجديد لا تظهر له خيارات الشحن والسحب حتى يتم القبول
    keyboard = [
...
// التعامل مع أزرار القبول والرفض من طرف الأدمن
bot.on('callback_query', async (query) => {
  ...
  if (data.startsWith('APPROVE_')) {
    ...
    approvedUsers.add(targetChatId);
  ...
  if (data.startsWith('REJECT_')) {
    ...
    bot.sendMessage(targetChatId, "❌ نعتذر منك، تم رفض طلب إنشاء الحساب من قبل الإدارة. يرجى التواصل مع الدعم للمزيد من التفاصيل.");
  }
});
...
bot.on('message', async (msg) => {
  ...
  // التحقق من صلاحية الشحن والسحب
  if (text.includes('شحن حساب') || text.includes('سحب رصيد')) {
    if (!approvedUsers.has(chatId)) {
      return bot.sendMessage(chatId, "⚠️ يجب عليك أولاً إنشـاء حساب وانتظار موافقة الإدارة قبل استخدام خدمات الشحن والسحب.");
    }
    return paymentKeyboard(chatId, text.includes('شحن') ? 'شحن' : 'سحب');
  }

  switch (text) {
    case '➕ إنشاء حساب جديد':
      if (approvedUsers.has(chatId)) {
        return bot.sendMessage(chatId, "✅ لديك حساب مفعّل بالفعل بالمنصة!");
      }
  ...
  if (session) {
    ...
    // 1. خطوات إنشاء الحساب وإرساله للأدمن للقبول/الرفض
    ...
    if (session.step === 'CREATE_PASS') {
      ...
      // إرسال طلب تفعيل الحساب للأدمن مع أزرار (قبول / رفض)
      const adminOptions = {
        parse_mode: 'Markdown',
        reply_markup: {
          inline_keyboard: [
      ...
      delete userSessions[chatId];
      return bot.sendMessage(chatId, `⏳ تم إرسال طلب إنشاء الحساب إلى الإدارة.\nسيتم مراجعة طلبك وإعلامك بالنتيجة فوراً (يرجى الانتظار).`);
    }

    // 2. إدخال الشحن (شام كاش)
    ...
    if (session.step === 'SHAM_AMOUNT') {
      ...
      // إشعار للأدمن بتفاصيل عملية الإيداع
      ...
      delete userSessions[chatId];
      bot.sendMessage(chatId, "✅ تم إرسال طلب الشحن بنجاح وإشعار الدعم للتحقق والتحويل.");
      return mainKeyboard(chatId);
    }

    // 3. إدخال الشحن (سيريتل كاش)
    ...
    if (session.step === 'WITHDRAW_AMOUNT') {
      ...
      // إشعار للأدمن بتفاصيل عملية السحب
